...
class CreateOrders(tk.Toplevel):
    ...
    def callback_table_num(self, P):
        if P == "":
            return True  # Allow clearing the entry box without triggering logic

        if not P.isdigit():
            messagebox.showerror("Input Error", "Please enter a valid table number.")
            self.tb_name_entry.delete(0, tk.END)
            return False
        
        table_num = int(P)
        if table_num > self.fac_info[1]:
            messagebox.showerror("Input Error", f"Maximum number of tables is {self.fac_info[1]}.")
            self.tb_name_entry.delete(0, tk.END)
            return False

        try:
            query = "SELECT * FROM orders WHERE table_num = ?"
            result = self.fac_db.read_val(query, (table_num,))

            if not result:
                # messagebox.showinfo("No Records", f"No orders found for table {table_num}.")
                return True

             # Clear existing widgets in the frame
            for widget in self.pr_sel_canvas_frm.winfo_children():
                widget.destroy()
            ...
            # Display records inside the frame
            for idx, r in enumerate(result):
                order_text = f"Item: {r[2]} \t\t| Qty: {r[3]} \t\t| Status: ordered"
                tk.Label(self.pr_sel_canvas_frm, text=order_text, anchor="center", justify="center", bg="grey").grid(
