...
class PrintOrders(tk.Toplevel):
    ...
    def load_orders(self):
        try:
            ...
            if self.t_num:
                load_query = """SELECT id, product_name, SUM(order_quantity) as order_quantity, SUM(order_price) as order_price FROM cooked_orders WHERE table_num = ? AND print_info=0 GROUP BY product_name"""
    ...
    def print_receipt(self):
        ...
        with open("ordertemplate.html") as html_doc:
        ...
        update_query = """UPDATE cooked_orders
                SET print_info = 1
                WHERE table_num = ?
                """
        self.fac_db.update(
                    update_query, (self.t_num))
        self.clear_all()
